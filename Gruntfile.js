...
module.exports = function(grunt) {
    ...
    grunt.registerTask("init-css", "初始化css", function( name, noSprite) {
        var app,
            src_path,
            result;
    ...
    grunt.registerTask("dist-css", "编译css", function(name) {
        ...
        // 定义编译路径
        dist_path = src_path.replace(config.src_path, config.dist_path);
        ...
        if(app.sprite){
            ...
            app.sprite.forEach(function(val, index){
                var sprite_src = get_path_str(dist_path + val.src);
                ...
                options.forEach(function(val2, index2){
                    ...
                    if(val2.copy === void 0 || !val2.copy === false){
                        ...
                        grunt.config.set('copy.sprite-img-'+ index + index2, {
                            files: [{
                                ...
                                expand: true,
                                src: '*.png',
                                dest: val2.imagepath.replace(config.src_path, config.dist_path),
                                filter: 'isFile'
                            }]
                        });
                    ...
                    //替换变量 src => dist
                    val2.imagepath = val2.imagepath ? val2.imagepath.replace(config.src_path, config.dist_path) : '';
                    val2.spritedest = val2.spritedest ? val2.spritedest.replace(config.src_path, config.dist_path) : '';
        ...
        if(app.dest){//如果有依赖处理
            ...
            if (app.src) {//如果有源
                result.src = get_path_arr(app.src, dist_path);
            }
            ...
            if(app.sprite){//如果有sprite图
                ...
                app.sprite.forEach(function(val){//这里的sprite css已经在dist层里, 且处理过合并img了
                    val.src && result.src.push(get_path_str(dist_path + val.src));
                });
            ...
            if(result.src && result.src.length){
                ...
                grunt.config.set('cssmin.dest', {
                    ...
                    src: get_path_str(dist_path + app.dest),
                    dest: get_path_str(dist_path + app.dest)
                });
    ...
    grunt.registerTask("dist-js", "编译js", function(name) {
        ...
        if(app.src){
            result = {
                src: get_path_arr(app.src, src_path)
            }
        ...
        if(app.noCmd){
            ...
            result.src = result.src.concat(get_path_arr(app.noCmd, src_path));
        }
        ...
        if(app.dest){
            ...
            if (app.noCmd) {
                result.src = get_path_arr(app.noCmd, dist_path);
            ...
            if (app.src) {
                ...
                result.src = result.src.concat(get_path_arr(app.src, dist_path));
                // result.src.push.apply(result.src, get_path_arr(app.src, dist_path));
            }
    ...
    grunt.registerTask("dist-copy", "复制文件到dist层", function(name){
        ...
        grunt.config.set("copy.dist", {
            files: [{
                ...
                cwd: src_path,
                src: app.src,
                dest: src_path.replace(config.src_path, config.dist_path),
                filter: 'isFile'
            }]
        });
